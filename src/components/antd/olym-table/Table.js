...
import SelectionBox from '../table/SelectionBox';
import SelectionCheckboxAll from '../table/SelectionCheckboxAll';
import Column from '../table/Column';
import ColumnGroup from '../table/ColumnGroup';
import { flatArray, treeMap, flatFilter, normalizeColumns } from '../table/util';
function noop() {
}
...
const defaultLocale = {
    ...
    filterConfirm: '确定',
    filterReset: '重置',
    emptyText: React.createElement("span", null,
...
export default class Table extends React.Component {
    constructor(props) {
        ...
        this.renderSelectionBox = (type) => {
            return (_, record, index) => {
                ...
                return (React.createElement("span", { onClick: stopPropagation },
                    React.createElement(SelectionBox, { type: type, store: this.store, rowIndex: rowIndex, disabled: props.disabled, onChange: handleChange, defaultSelection: this.getDefaultSelection() })));
            };
        };
        ...
        //将设定table选中行keys方法传出
        this.props.setSelectedRowKeys ? this.props.setSelectedRowKeys(this.outerSetSelectedRowKeys) : ''
    }

    //新增一个方法来设定table的选中行keys
    outerSetSelectedRowKeys = (selectedRowKeys) => {
    ...
    getLocale() {
        ...
        return Object.assign({}, defaultLocale, locale, this.props.locale);
    }
    ...
    renderColumnsDropdown(columns) {
        ...
        return treeMap(columns, (originColumn, i) => {
            ...
            if ((column.filters && column.filters.length > 0) || column.filterDropdown) {
                ...
                filterDropdown = (React.createElement(FilterDropdown, { locale: locale, column: column, selectedKeys: colFilters, confirmFilter: this.handleFilter, prefixCls: `${prefixCls}-filter`, dropdownPrefixCls: dropdownPrefixCls || 'ant-dropdown', getPopupContainer: this.getPopupContainer }));
            }
            if (column.sorter) {
                let isSortColumn = this.isSortColumn(column);
                if (isSortColumn) {
                ...
                sortButton = (React.createElement("div", { className: `${prefixCls}-column-sorter` },
                    React.createElement("span", { className: `${prefixCls}-column-sorter-up ${isAscend ? 'on' : 'off'}`, title: "↑", onClick: () => this.toggleSortOrder('ascend', column) },
                        React.createElement(Icon, { type: "caret-up" })),
                    React.createElement("span", { className: `${prefixCls}-column-sorter-down ${isDescend ? 'on' : 'off'}`, title: "↓", onClick: () => this.toggleSortOrder('descend', column) },
                        React.createElement(Icon, { type: "caret-down" }))));
            }
            column.title = (React.createElement("span", null,
                column.title,
                sortButton,
                filterDropdown));
            return column;
    ...
    renderPagination() {
        ...
        return (total > 0) ? (React.createElement(Pagination, Object.assign({ key: "pagination" }, pagination, { className: classNames(pagination.className, `${this.props.prefixCls}-pagination`), onChange: this.handlePageChange, total: total, size: size, current: this.getMaxCurrent(total), onShowSizeChange: this.handleShowSizeChange }))) : null;
    }
    ...
    render() {
        ...
        const table = (React.createElement(RcTable, Object.assign({ key: "table" }, restProps, { prefixCls: prefixCls, data: data, columns: columns, showHeader: showHeader, className: classString, expandIconColumnIndex: expandIconColumnIndex, expandIconAsCell: expandIconAsCell, emptyText: () => locale.emptyText })));
